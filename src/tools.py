...
class GraphQueryAgent():
    ...
    def run(self, query):
        ...
        if "Final Answer:" in result['output']:
            ...
            response = split_output[1].strip() if len(split_output[1].strip()) > 1 else "N/A"
        else:
...
class SQLQueryAgent():
    ...
    def run(self, query: str):
        '''Run agent with user query.'''
        result = self.agent_exec.invoke({"input": query, "schema": self.db.table_info, "tools": self.tools})
        # parse the output
        if "Final Answer:" in result['output']:
            split_output = re.split('(?i)Final Answer:', result['output'])
            response = split_output[1].strip() if len(split_output[1].strip()) > 1 else "N/A"
